...
export default function Home() {
    ...
    useEffect(() => {
        ...
        // CASE 1: If "Show Bookmarked Only" and no query → load bookmarked repos
        if (showBookmarkedOnly && !debouncedQuery.trim()) {
            ...
            Promise.all(
                bookmarked.map(id =>
                    fetch(`https://api.github.com/repositories/${id}`).then(res => res.json())
                )
            )
                .then(results => {
                    if (!mounted) return;
                    setRepos(results.filter(r => r && !r.message)); // filter out invalid responses
                })
                .catch(err => {
                    if (!mounted) return;
        ...
        // CASE 2: If query is empty and not showing bookmarked → clear
    ...
    }, [debouncedQuery, showBookmarkedOnly, bookmarked]);
