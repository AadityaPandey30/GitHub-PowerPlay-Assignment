...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (showBookmarkedOnly && !debouncedQuery.trim()) {
      ...
      Promise.all(
        bookmarked.map((id) =>
          fetch(`https://api.github.com/repositories/${id}`).then(
            async (res) => {
    ...
    debouncedQuery,
    showBookmarkedOnly /* bookmarked intentionally omitted */,
  ]);
    ...
    <div className="min-h-screen flex flex-col">
        ...
        <div className="max-w-4xl mx-auto px-4 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            <Toggle
              ...
              onChange={setShowBookmarkedOnly}
              label="All Bookmarked Only"
            />
          </div>
        </div>
      ...
      <footer className="py-6 px-3 text-center text-sm text-gray-500">
        ...
        <a
          className="underline"
          target="_blank"
          href="https://www.linkedin.com/in/aaditya-pandey-ab2829257"
        >
          Aaditya
        </a>{" "}
        • Uses GitHub public search API (30 results)
      </footer>
    </div>
  );
}
